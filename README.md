...
模型和观察点使用两个独立的 z：世界坐标中屏幕平面固定为 `z=0`，模型中心的 z 负值表示在屏幕后方，正值表示跑到屏幕前方；观察点 z 是眼睛到屏幕的距离，始终为正，决定离轴透视和远近变化。模型 z 不是摄像头返回的 z，也不是“眼睛在屏幕上的 z”。

开启网页的“按实测尺寸投影”后，模型设置会显示“屏幕后深度（米）”，程序自动换算为场景 z。请测量可见画面宽度和校准时眼屏距离后再开启；不要为了让缩放更明显而把模型手动调到正 z，那会把物体放到屏幕外侧。物体距离屏幕越深，前后移动时的透视差越明显；头部 z 的响应仍由物理模式统一换算，演示模式则由“纵深跟随增益”控制。

注意：对屏幕后方的真实物体，屏幕边框固定时，靠近屏幕意味着看到更大的空间范围，物体在屏幕中的相对尺寸不一定变大。若产品要求“靠近就放大、远离就缩小”，那属于额外的非物理 zoom 效果，应该单独实现，不能用正的模型 z 混淆坐标含义。
